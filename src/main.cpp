...
int main() {
  ...
  int lane = 1; //car starts in lane 1
  double ref_vel = 0.0; // speed mph
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          /***
          PREDICTION
          The prediction component estimates what actions other objects might take in the future.In this prediction module,
           we use three status,car head,car left,car right, and find out close following.
          ***/

          int previous_size = int(previous_path_x.size());
          if (previous_size > 0) {
            car_s = end_path_s;
          }
